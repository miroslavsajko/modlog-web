...
import {
    ChartData,
    ModEntriesAPIResponse,
    ModlogAPIResponse,
    PostsAPIResponse
} from "../types/interfaces.ts";
import {getActionGroupForModAction} from "../types/translations.ts";
...
export const fetchModEntries = async ({
                                          sort = 'timestamp',
                                          sortDirection = 'desc',
                                          page = 0,
                                          pageSize = 15,
    ...
    return await axios.get<ModlogAPIResponse>(apiUrl).then(response => {
        response.data.entries.forEach(entry => {
            const actionGroup = getActionGroupForModAction(entry.action);
            if (actionGroup === 'POST_OR_COMMENT') {
                if (entry.commentid) {
                    entry.type = 'COMMENT';
                    return
                }
            ...
            entry.type = actionGroup ?? 'GENERAL'
        })
